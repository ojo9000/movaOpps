<%
call mplSearchView(req("Object_id")&"", req("no")&"",req("id")& "",req("param1")& "",req("param2")& "" ,"ok")

sub mplSearchView(mplObject,mplNo,mplId,param1,param2,mplTitle)
Dim OutListString
OutListString = ""
'modi code:
...
if req("param1")<>"" then 
	session("mplParam1")=req("param1")
end if 
if req("param2")<>"" then 
	session("mplParam2")=req("param2")
end if 


if req("no")<>"" then 
	session("search_no") =  req("no")
	session("object_id") =  req("object_id")
...
OutListString = OutListString & "		  <FORM id=form method=post name=mainform action=m.asp>"
...
OutListString = OutListString & "			 			<input class=cn_input03 id="& chr(34) & FRM_field_code & chr(34) &" type="&chr(34)& "hidden" &chr(34) & "  name="&chr(34)& FRM_field_code &chr(34)& " value="& chr(34) & curValue & chr(34) & ">"
		...
		if FRM_search_type="dateRange" then
			...
			mplPreCondition(tempM,3) = FLD_field_code
					OutListString = OutListString &  "<TD class=cn_td10 align='left'>"
					OutListString = OutListString &  "	从 <INPUT  READONLY ondblclick="&chr(34)& "this.value=''"&chr(34)& " size="&chr(34)& "10"&chr(34)& " class=cn_input03 type=text name="&chr(34)& "__S_"& FRM_field_code &chr(34) & " id="&chr(34)& "__S_" & chr(34) & FRM_field_code &  chr(34) & ">" & getCalendar("__S" & FRM_field_code,"__S_" & FRM_field_code,"D")
	...
	movaRS.close
		 ...
		 if pre_conditionCount>0 then
		 	OutListString = OutListString &  "<BUTTON class=cn_button01 type=submit onClick="&chr(34)& "this.form.target='';this.form.action='m.asp'"&chr(34)& ">查询</button> "
		 end if 
...
OutListString = OutListString &  "  <TR>"
OutListString = OutListString & ""&  "    <TD colSpan=100>" & PageNavigatorString &"</TD></TR>  </TABLE>"
OutListString = OutListString &  " </div>"
